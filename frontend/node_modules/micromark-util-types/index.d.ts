 ...
 * `document` is the highest level of content, so it鈥檚 not listed here.
 *
 ...
 * It鈥檚 a weird one, and needed to make certain edge cases around definitions
 * spec compliant.
 ...
 * not depends on whether it鈥檚 defined.
 ...
 * `_bufferIndex`).
 *
 * `_bufferIndex` is `-1` when `_index` points to a code chunk and it鈥檚 a
...
export interface Token {
   ...
   * can鈥檛 be used (because links in links are incorrect).
   */
   ...
   * balanced: it鈥檚 not a link opening but has a balanced closing.
   */
 ...
 *
 * Tokens can 鈥渃ontain鈥?other tokens, even though they are stored in a flat
 ...
 * The reason being: well, there isn鈥檛 any state that makes sense, so
...
export interface Construct {
   ...
   * ````
   *
   * 鈥hen `- list?` cannot form if this fenced code construct is concrete.
   *
   ...
   *
   * For example, if we鈥檙e currently parsing a link title and this construct
...
export interface TokenizeContext {
   ...
   * 鈥淣ormal鈥?rules are, for example, that an empty list item can鈥檛 interrupt:
   *
   * ````markdown
   * a
   * *
   ...
   * These rules don鈥檛 apply to table body rows:
   *
   * ````markdown
   * | a |
   * | - |
  ...
  /**
   * Whether we鈥檙e currently interrupting.
   *
   * Take for example:
   *
   * ```markdown
   ...
   * Define a skip
   *
   * As containers (block quotes, lists), 鈥渘ibble鈥?a prefix from the margins,
...
export interface CompileContext {
  ...
  /**
   * Output an extra line ending if the previous value wasn鈥檛 EOF/EOL.
   *
   * @returns
   *   Nothing.
   */
...
export interface CompileData {
  ...
  /**
   * Whether we鈥檙e in fenced code, used by code (fenced).
   */
  ...
  /**
   * Whether we鈥檝e seen code data, used by code (fenced, indented).
   */
  ...
  /**
   * Whether we鈥檙e in code data, used by code (text).
   */
  ...
  /**
   * Stack of containers, whether they鈥檙e tight or not.
   */
...
export interface CompileOptions {
   ...
   *
   * URLs that have no protocol (which means it鈥檚 relative to the current page,
