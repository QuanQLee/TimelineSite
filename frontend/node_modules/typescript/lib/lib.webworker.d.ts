...
/**
 * The Client聽interface represents an executable context such as a Worker, or a SharedWorker. Window clients are represented by the more-specific聽WindowClient. You can get聽Client/WindowClient聽objects from methods such as Clients.matchAll() and聽Clients.get().
 *
...
/**
 * Provides access to聽Client聽objects. Access it聽via self.clients聽within a聽service worker.
 *
...
/**
 * This ServiceWorker API interface represents the event object of a message event fired on a service worker (when a channel message is received on the ServiceWorkerGlobalScope from another context) 鈥?extends the lifetime of such events.
 *
...
/**
 * This is the event type for fetch聽events dispatched on the聽service worker global scope. It contains information about the fetch, including the聽request and how the receiver will treat the response. It provides the event.respondWith() method, which allows us to provide a response to this fetch.
 *
...
/**
 * This Web Workers API interface is an interface representing the scope of any worker. Workers have no browsing context; this scope contains the information usually conveyed by Window objects 鈥?in this case event handlers, the console or the associated WorkerNavigator object. Each WorkerGlobalScope has its own event loop.
 *
