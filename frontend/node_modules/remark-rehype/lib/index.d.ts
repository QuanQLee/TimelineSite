 ...
 *
 * > 馃憠 **Note**:
 * > It鈥檚 highly unlikely that you want to pass a `processor`.
 *
 * ###### HTML
 *
 * Raw HTML is available in mdast as `html` nodes and can be embedded in hast
 * as semistandard `raw` nodes.
 * Most plugins ignore `raw` nodes but two notable ones don鈥檛:
 *
 ...
 * ###### Unknown nodes
 *
 * Unknown nodes are nodes with a type that isn鈥檛 in `handlers` or `passThrough`.
 * The default behavior for unknown nodes is:
 *
 * * when the node has a `value`
 *   (and doesn鈥檛 have `data.hName`, `data.hProperties`, or `data.hChildren`,
 *   see later),
