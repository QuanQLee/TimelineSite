...
export interface StandardLonghandProperties<TLength = (string & {}) | 0, TTime = string & {}> {
  ...
  /**
   * The **`content-visibility`** CSS property controls whether or not an element renders its contents at all, along with forcing a strong set of containments, allowing user agents to potentially omit large swathes of layout and rendering work until it becomes needed. It enables the user agent to skip an element's rendering work (including layout and painting) until it is needed 鈥?which makes the initial page load much faster.
   *
  ...
  /**
   * The **`shape-outside`** CSS property defines a shape鈥攚hich may be non-rectangular鈥攁round which adjacent inline content should wrap. By default, inline content wraps around its margin box; `shape-outside` provides a way to customize this wrapping, making it possible to wrap text around complex objects rather than simple boxes.
   *
  ...
  /**
   * The **`text-overflow`** CSS property sets how hidden overflow content is signaled to users. It can be clipped, display an ellipsis ('`鈥'), or display a custom string.
   *
  ...
  /**
   * The **`view-transition-name`** CSS property provides the selected element with a distinct identifying name (a `<custom-ident>`) and causes it to participate in a separate view transition from the root view transition 鈥?or no view transition if the `none` value is specified.
   *
   * **Syntax**: `none | <custom-ident>`
   *
   * **Initial value**: `none`
...
export interface StandardShorthandProperties<TLength = (string & {}) | 0, TTime = string & {}> {
  ...
  /**
   * The **`overflow`** CSS shorthand property sets the desired behavior for an element's overflow 鈥?i.e. when an element's content is too big to fit in its block formatting context 鈥?in both directions.
   *
...
export interface VendorLonghandProperties<TLength = (string & {}) | 0, TTime = string & {}> {
  ...
  /**
   * The **`-ms-ime-align`** CSS property is a Microsoft extension aligning the Input Method Editor (IME) candidate window box relative to the element on which the IME composition is active. The extension is implemented in Microsoft Edge and Internet Explorer聽11.
   *
   * **Syntax**: `auto | after`
   *
   * **Initial value**: `auto`
