 ...
 * controversially, character escapes and references don鈥檛 work in this
 * matching: `&copy;` does not match `漏`, and `\+` does not match `+`.
 *
 * But casing is ignored (and whitespace) is trimmed and collapsed: ` A\nb`
 * matches `a b`.
 * So, we do prefer the label when figuring out how we鈥檙e going to serialize:
 ...
 * serializing whether we鈥檙e for example in a double (`"`) or single (`'`)
 * quoted title.
 */
 ...
 *
 * This info isn鈥檛 used yet but such functionality will allow line wrapping,
 * source maps, etc.
 *
 * @param info
 *   Info on where we are in the document we are generating.
...
/**
 * Whether to encode things 鈥?with fields representing the surrounding of a
 * whole.
 */
...
/**
 * Whether to encode things 鈥?with fields representing the relationship to a
 * whole.
 */
 ...
 *
 * 鈥淏locks鈥?are typically joined by one blank line.
 * Sometimes it鈥檚 nicer to have them flush next to each other, yet other
 ...
 *   ```
 *
 *    > 馃憠 **Note**: abusing this feature will break markdown.
...
export interface Options {
  /**
   * Marker to use in certain cases where the primary bullet doesn鈥檛 work
   ...
   *
   * The default is to add blank lines between any flow (鈥渂lock鈥? construct.
